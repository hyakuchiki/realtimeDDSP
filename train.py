...
def main(cfg):
    ...
    if "xfer_db_path" in cfg:
        cfg.xfer_db_path = hydra.utils.to_absolute_path(cfg.xfer_db_path)
    if "xfer_raw_dir" in cfg:
        cfg.xfer_raw_dir = hydra.utils.to_absolute_path(cfg.xfer_raw_dir)
    # load model
    ...
    xfer_dataset = hydra.utils.instantiate(
        cfg.data, raw_dir=cfg.xfer_raw_dir, db_path=cfg.xfer_db_path
    )
    ...
    xfer_dl = DataLoader(
        xfer_dataset, cfg.batch_size, shuffle=False, num_workers=cfg.num_workers
    )
    # trainer setup
    ...
    # train model
    trainer.fit(model, train_dl, [valid_dl, xfer_dl], ckpt_path=cfg.ckpt)
